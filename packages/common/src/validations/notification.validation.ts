...
import { SelectionOptionEnum } from '../types/survey.types';
...
const commonAdminNotification = z
    .object({
    ...
    .superRefine((data, ctx) => {
        ...
        if (isAnyFieldProvided && !areAllFieldsProvided) {
            ctx.addIssue({
                ...
                message:
                    errorMessage.ADMIN_NOTIFICATION.INCOMPLETE_SCHEDULE_FIELDS,
        ...
        if (
            areAllFieldsProvided &&
            !isFutureDate(schedule_date, schedule_hour, schedule_minute)
        ) {
            ctx.addIssue({
                code: z.ZodIssueCode.custom,
