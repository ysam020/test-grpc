...
const { sendEmail, tokenFns, hashFns } = require('@atc/common');
...
describe('Reset Password Handler', () => {
    ...
    beforeEach(() => {
        jest.clearAllMocks();
        mockCallback = jest.fn();
        process.env.RESET_JWT_TOKEN = 'reset-secret';
    });
    ...
    it('should reset password when token and OTP are valid', async () => {
        tokenFns.verifyToken.mockReturnValue({ email: mockUser.email });
        (getUserByEmail as jest.Mock).mockResolvedValue(mockUser);
        hashFns.hashValue.mockResolvedValue('hashed-pass');

        await resetPassword(mockCall as any, mockCallback);
    ...
    it('should return error if user not found', async () => {
        tokenFns.verifyToken.mockReturnValue({ email: 'notfound@example.com' });
        (getUserByEmail as jest.Mock).mockResolvedValue(null);
    ...
    it('should return error if OTP does not match', async () => {
        const wrongOtpUser = { ...mockUser, otp: 654321 };
        tokenFns.verifyToken.mockReturnValue({ email: wrongOtpUser.email });
        (getUserByEmail as jest.Mock).mockResolvedValue(wrongOtpUser);
